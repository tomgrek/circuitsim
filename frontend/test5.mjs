...
E1 th_out 0 VOL={V(6)>V(5) ? 1 : 0}
E2 tr_out 0 VOL={V(2)<V(61) ? 1 : 0}

G1 0 state VALUE={V(tr_out)*1m - V(th_out)*1m}
C1 state 0 10p
D1 state limit_high DCLAMP
D2 limit_low state DCLAMP
V1 limit_high 0 DC 5
V2 limit_low 0 DC 0
.MODEL DCLAMP D(IS=1e-6 N=0.01)

E_OUT 3 1 VOL={V(state)>2.5 ? V(8) : 0}
S_DIS 7 1 8 state SMOD_DIS
.MODEL SMOD_DIS SW(VT=2.5 RON=1 ROFF=100MEG)
.ENDS NE555
